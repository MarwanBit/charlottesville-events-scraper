...
def parse_event_datetime_range(
    ...
    """Parse e.g. 'Saturday, April 4, 2026 7:00pm - 9:00pm' into a list of (start_date, end_date, start_time, end_time).
    Multiple time ranges on one day return multiple tuples (one per range).
    Date-only returns one all-day range (00:00–23:59:59). Parse failure returns [(None, None, None, None)]."""
        ...
        if start_date is not None:
            ...
            for start_t, end_t in ranges:
                try:
                    ...
                    result.append((start_date, start_date, t0, t1))
                except ValueError:
                    continue
            if result:
                return result
